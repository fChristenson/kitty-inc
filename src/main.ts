...
import {
  ...
  wireFloorBuyOpenBookCritButton,
...
async function main() {
  ...
  if (import.meta.env.MODE !== "production") {
    ...
    wireSpawnDominoEffectCritButton(app, () => {
      const floor = (buildings[activeBuildingIndex] ?? [])[0];
      if (floor) forceDominoEffectCritUpgrade(floor, "crit");
    });
    ...
    wireSpawnExecutiveBonusCritButton(app, () => {
      const floor = (buildings[activeBuildingIndex] ?? [])[0];
      if (floor) forceExecutiveBonusCritUpgrade(floor);
    });
    wireSpawnPowerSurgeCritButton(app, () => {
      const floor = (buildings[activeBuildingIndex] ?? [])[0];
      if (floor) forcePowerSurgeCritUpgrade(floor);
    });
    ...
    wireSpawnOpenBookCritButton(app, () => {
      const floor = (buildings[activeBuildingIndex] ?? [])[0];
      if (floor) forceOpenBookCritUpgrade(floor);
    });
    ...
    wireFloorBuyOpenBookCritButton(app, () =>
      forceOpenBookFloorBuyCrit("crit"),
    );
