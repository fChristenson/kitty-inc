...
// loads the skyline art; main.ts awaits this alongside loadFloorBackgrounds
// before the first frame ever needs to draw it
export async function loadCityImage(): Promise<HTMLImageElement> {
  cityImage = await loadImageByName("city");
  return cityImage!;
}

// the sky gradient's own ground-band color (see gameCanvas.ts) must match city.png's
// own baked-in top-edge color
export function getCitySkyGroundColor(): string {
  return COLOR.skyGround;
}
