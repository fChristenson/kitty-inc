...
import { loadThemeSprite, type ThemeName } from "../../loadAssets";
...
// cell shape (see drawFigure). Scaled by ROOM_CONTENT_SCALE (like the room art and
// WORKER_FEET_Y already are) so the cat stays sized proportionately to the room now
// that the room's own art is compressed to fit the shorter floor band — this was
// left as a flat 480 when ROOM_CONTENT_SCALE was introduced later, making the cat
// render nearly as tall as the whole floor band ("sprite is massive" report).
const RENDER_H = 330 * ROOM_CONTENT_SCALE;

// per-theme manager render-size multiplier (applied on top of RENDER_H, see
// drawFigure/drawWorker) — for themes whose own manager sprite reads noticeably
// smaller/larger than its worker counterpart once actually loaded in-game.
// Absent entries just use 1 (no adjustment).
const MANAGER_SCALE_BY_THEME: Partial<Record<ThemeName, number>> = {
  "bakery-cafe": 1.15,
};
...
// whichever theme's sprites are CURRENTLY active (see loadWorkerSprite below) —
// needed by drawWorker to apply MANAGER_SCALE_BY_THEME's per-theme manager scale
let activeTheme: ThemeName = "references";

// loads (or reuses an already-cached) theme's base sprite sheets and makes them
// the active pair drawWorker reads from — call whenever the active building's
// own theme changes, not just once
const spritesByTheme = new Map<
  ThemeName,
  { worker: HTMLImageElement | null; manager: HTMLImageElement | null }
>();
export async function loadWorkerSprite(
  theme: ThemeName = "references",
): Promise<HTMLImageElement> {
  activeTheme = theme;
  const cached = spritesByTheme.get(theme);
  if (cached) {
    workerSprite = cached.worker;
    managerSprite = cached.manager;
    return workerSprite!;
  }
  const [worker, manager] = await Promise.all([
    loadThemeSprite(theme, "worker"),
    loadThemeSprite(theme, "manager"),
  ]);
  spritesByTheme.set(theme, { worker, manager });
  workerSprite = worker;
...
// Cropped once per theme's own sprite and cached; returns null until
// loadWorkerSprite has resolved for the currently-active theme
export function getWorkerIconUrl(): string | null {
  if (!workerSprite) return null;
  const cached = workerIconUrlBySprite.get(workerSprite);
  if (cached) return cached;
  const url = cropCameraFacingIconUrl(workerSprite);
  workerIconUrlBySprite.set(workerSprite, url);
  return url;
}

// the "hire manager"/manager-boost icons (hud/upgradeMenu, hud/corporationBoostMenu)
// — see cropCameraFacingIconUrl. Cropped once per theme's own sprite and cached;
// returns null until loadWorkerSprite has resolved for the currently-active theme
// (it loads managerSprite too)
export function getManagerIconUrl(): string | null {
  if (!managerSprite) return null;
  const cached = managerIconUrlBySprite.get(managerSprite);
  if (cached) return cached;
  const url = cropCameraFacingIconUrl(managerSprite);
  managerIconUrlBySprite.set(managerSprite, url);
  return url;
}

// preloads (once) the references theme's own manager sprite into spritesByTheme
// WITHOUT touching the currently-active workerSprite/managerSprite/activeTheme —
// call once during app bootstrap so getReferenceManagerIconUrl (below) always has
// something ready by the time it's first rendered
export async function preloadReferenceManagerSprite(): Promise<void> {
  if (spritesByTheme.has("references")) return;
  const [worker, manager] = await Promise.all([
    loadThemeSprite("references", "worker"),
    loadThemeSprite("references", "manager"),
  ]);
  spritesByTheme.set("references", { worker, manager });
}

// the "Hold press conference" menu item (hud/corporationBoostMenu) is a GLOBAL
// action, not tied to any one company/building (same reasoning as the coin/mouse/
// audience icons elsewhere, which are always hardcoded to "references" too) — so
// its icon must always show the REFERENCES manager, never whichever theme happens
// to be active for the currently-viewed building. See preloadReferenceManagerSprite.
export function getReferenceManagerIconUrl(): string | null {
  const sprite = spritesByTheme.get("references")?.manager;
  if (!sprite) return null;
  const cached = managerIconUrlBySprite.get(sprite);
  if (cached) return cached;
  const url = cropCameraFacingIconUrl(sprite);
  managerIconUrlBySprite.set(sprite, url);
  return url;
}
...
export function getBoostedWorkerCenters(
  ...
  const state = floorWorkers.get(floor);
  if (!state) return [];
  // nudged 20px above the worker's own center per an explicit visual tweak request
  const y = WORKER_FEET_Y - RENDER_H / 2 - 150;
...
// one recolored frame per (sprite, frame, tintIndex) is built once and reused
// forever after that — cheap since it's at most 5 frames * THEME_COLORS.length
// canvases per sprite. Keyed by the sprite object itself (not just "w"/"m" role)
// so switching themes never serves a stale recolor from a DIFFERENT sprite that
// happened to reuse the same frame+tintIndex — this bit once already (see
// getWorkerIconUrl/getManagerIconUrl's own sprite-identity-keyed cache below);
// a plain "role:frame:tint" string key here made every worker whose
// (frame, tintIndex) had already been cached under one theme silently keep
// rendering that OLD theme's art forever after switching to a new theme/company,
// reported as "adding a worker in the tech theme spawned a reference worker".
const recoloredFrameCache = new WeakMap<
  HTMLImageElement,
...
function getRecoloredFrame(
  ...
  if (!sprite) return null;
  let bySprite = recoloredFrameCache.get(sprite);
  if (!bySprite) {
    bySprite = new Map();
    recoloredFrameCache.set(sprite, bySprite);
  }
...
function drawFigure(
  ...
  if (!recolored) return;
  const renderH = RENDER_H * scaleMultiplier;
...
export function drawWorker(
  ...
  state.walkers.forEach((walker, i) => {
    ...
    drawFigure(
      ...
      // bakery-cafe's own manager (baker's toque) art reads noticeably smaller
      // than its worker counterpart once loaded in-game — bumped up 15% per an
      // explicit visual tweak request, scoped to just that theme's manager
      i === managerWalkerIndex ? (MANAGER_SCALE_BY_THEME[activeTheme] ?? 1) : 1,
    );
  });
}
