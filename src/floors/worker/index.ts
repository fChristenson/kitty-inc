...
export function getWorkerPermaTier(
  ...
  return workerIndex === managerIndexOf(floor)
    ? (floor.managerPermaTier ?? null)
...
// workers/manager a "Boost" event could still promote
export function getBoostEventCandidates(floor: Floor): number[] {
  const count = getRenderedWorkerCount(floor);
  const indexes: number[] = [];
  for (let i = 0; i < count; i++) {
    if (getWorkerPermaTier(floor, i) !== CRIT_TIER_ORDER[0]) indexes.push(i);
  }
  return indexes;
}
...
// the one walker a running "Boost" event draws itself (see
// drawWorkerSpotlight), so drawWorker leaves it out of the normal pass
let spotlight: { floor: Floor; workerIndex: number } | null = null;

export function setWorkerSpotlight(floor: Floor, workerIndex: number): void {
  spotlight = { floor, workerIndex };
}

export function clearWorkerSpotlight(): void {
  spotlight = null;
}

// draws the spotlighted walker frozen in its last drawn pose, its current
// tier's glow crossfading into the next tier's by promotion (0..1), tilted by
...
// the same jump + coin burst + timed boost a manager's auto-boost plays for
// each of its workers, for just this one
export function celebrateWorkerBoost(
  floor: Floor,
  workerIndex: number,
  now: number,
): void {
  ...
  if (!walker) return;
  activateBoosted(floor, workerIndex, now);
  walker.clickedAt = now;
  spawnCoinBurst(floor, walker.x, WORKER_FEET_Y - RENDER_H / 2, () => {});
  playAutoBoost();
}
...
function drawFigure(
  ...
  if (!recolored) return;
  const image = whiteAlpha > 0 ? whitenFrame(recolored, whiteAlpha) : recolored;
...
export function drawWorker(
  ...
  state.walkers.forEach((walker, i) => {
    ...
    const dtSeconds = Math.max(
      (now - Math.max(walker.lastUpdate, getScreenUnfrozenAt())) / 1000,
      0,
    );
    ...
    walker.pose = {
      groundY: WORKER_FEET_Y - bounce + walkBob,
