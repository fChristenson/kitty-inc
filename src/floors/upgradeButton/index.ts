...
//
// All of this run's state (overtimeTicks/overtimeStartedAt/overtimeCost) lives
// directly on the Floor object (gameState.ts), not a WeakMap here — so it's
// persisted (survives a reload) and travels with the floor across building
// switches. The drain tail is derived purely from (overtimeTicks,
// overtimeStartedAt, now), so it correctly keeps draining across however long
// the app was actually closed, same idea as gameState.ts's idle-income catch-up.
