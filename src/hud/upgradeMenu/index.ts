    ...
    <div class="worker-menu" id="upgrade-menu" hidden>
      ...
      <div class="worker-menu__panel">
        <div class="worker-menu__header">
          <h2>Upgrades</h2>
          <button class="worker-menu__close" id="upgrade-menu-close" aria-label="Close">&times;</button>
        </div>
...
// wires the menu's open/close controls, the "buy a new building" item, and the
// per-floor "Add new worker" buttons; getFloors/getBuildingCount are called fresh
// each render so the list always reflects live state (including a new floor unlocked
// or a new building bought while the menu happens to be open)
export function wireUpgradeMenu(
  ...
  function render(): void {
    ...
    const buildingPrice = getBuildingPrice(getBuildingCount());
    const buildingAffordable = getTotalIncome() >= buildingPrice;
    const buyBuildingItem = `
      <button
        class="worker-menu__item"
        id="upgrade-menu-buy-building"
        ${buildingAffordable ? "" : "disabled"}
      >
        <span>Buy a new building</span>
        <span class="worker-menu__price">${formatPrice(buildingPrice)}</span>
      </button>
    `;
    const floorItems = floors
      ...
      .join("");
    list.innerHTML = buyBuildingItem + floorItems;
  }

  list.addEventListener("click", (event) => {
    const target = event.target as HTMLElement;
    if (target.closest("#upgrade-menu-buy-building")) {
      if (buyBuilding()) {
        onPurchase();
        render();
      }
  ...
  function updateAffordability(): void {
    const buyBuildingButton = list.querySelector<HTMLButtonElement>(
      "#upgrade-menu-buy-building",
    );
    if (buyBuildingButton) {
      buyBuildingButton.disabled =
        getTotalIncome() < getBuildingPrice(getBuildingCount());
    }
    list
