...
`applyFloorCrit` path. Rewards are immediate and current-building only; no
...
`plushiePand` uses the dark-background threshold processor; the other sources
use the standard border-seeded near-white removal. Every raw `.jfif` remains
preserved, and dedicated `scripts/process-<name>.mjs` wrappers regenerate the
250px-capped palette PNGs in `public/`, `public/stickers/`, and
`public/silhouettes/`.

Verification: `node scripts/check-crit-names.mjs`,
`node scripts/test-featured-crits.mjs` and `npm run build` pass. The regression
suite now covers 613 crit rewards and validates the new asset properties.

## Implemented asset batch: 2026-09-22 (cocktail hour and cash piles)

Twenty-seven supplied illustrations support upgrade clicks and floor unlocks
...
Fourteen assets arrived already named against `docs/prompts.md`'s cocktail list
and kept those labels verbatim. The `2` variants and the money piles needed
their own identities, checked against the registry with
`node scripts/check-crit-names.mjs`.

| Image               | Crit                  | Immediate reward                                 | Proc chance |
| ------------------- | --------------------- | ------------------------------------------------ | ----------- |
| emptyGlass          | On the Rocks          | 19 instant payouts on this floor                 | 0.75%       |
| amberSpritz         | Amber Spritz          | 20 instant payouts on this floor                 | 0.7%        |
| copperMugMule2      | Mule Kick             | 21 instant payouts on this floor                 | 0.65%       |
| negroniNightfall2   | Bitter Bounty         | 24 instant payouts on this floor                 | 0.55%       |
| blackberryBramble   | Blackberry Bramble    | 22 free upgrades on this floor                   | 0.52%       |
| blackberryBramble2  | Bramble Bounty        | 21 free upgrades on every other floor            | 0.48%       |
| singaporeSling      | Singapore Sling       | 25 instant payouts on every other floor          | 0.42%       |
| derbyDayJulep       | Derby Day Julep       | 27 free upgrades on this floor                   | 0.39%       |
| derbyDayJulep2      | Frostcup Julep        | 26 payouts on the lowest-level floor             | 0.37%       |
| hurricaneHour       | Hurricane Hour        | 28 free upgrades on this floor                   | 0.36%       |
| cosmoCashout        | Cosmo Cashout         | 29 free upgrades on this floor                   | 0.34%       |
| frenchSeventyFive   | French Seventy-Five   | 26 free upgrades on every other floor            | 0.33%       |
| sidecarSurge        | Sidecar Surge         | 27 upgrades on the highest unlocked floor        | 0.31%       |
| allowance           | Allowance             | 30 payouts on the lowest-level floor             | 0.3%        |
| hurricaneHour2      | Eye of the Storm      | 31 instant payouts on every other floor          | 0.29%       |
| copperMugMule       | Copper Mug Mule       | 30 free upgrades on every unlocked floor         | 0.27%       |
| pineappleParadise   | Pineapple Paradise    | 31 upgrades here and every floor below           | 0.26%       |
| negroniNightfall    | Negroni Nightfall     | 30 upgrades here and on the highest floor        | 0.24%       |
| oldFashionedFortune | Old Fashioned Fortune | 34 payouts from the highest-earning floor        | 0.23%       |
| tikiZombie          | Tiki Zombie           | 33 instant payouts on every unlocked floor       | 0.21%       |
| tikiZombie2         | Tiki Torch            | 20 upgrades, then 20 payouts on this floor       | 0.19%       |
| allowance2          | Rainy Day Fund        | 32 free upgrades on every unlocked floor         | 0.18%       |
| lootBags            | Bagged and Tagged     | 38 upgrades on the highest unlocked floor        | 0.17%       |
| longIslandLandslide | Long Island Landslide | 36 instant payouts on every unlocked floor       | 0.15%       |
| pocketMoney2        | Petty Cash            | 37 instant payouts on every unlocked floor       | 0.13%       |
| liquidAssets2       | Fluid Capital         | 44 payouts from the highest-earning floor        | 0.12%       |
| pocketMoney         | Pocket Money          | 1 tier promotion, then 15 upgrades on this floor | 0.11%       |

Balance: each entry slots into an existing family and stays strictly monotonic
against its neighbours — bigger reward, rarer roll. The building-wide upgrade
pair sits above Queen of Queens' 28 at 1.3%; the downward-slice entry sits
between Frost Rune's 29 and Ironclad Guarantee's 33; the top-earner pair
brackets Piston Paladin's 41. Thirteen ordering families in the regression
script now pin all of this.

Scope: every reward is current-building only. Building-wide and alternating
targets skip locked floors. Lowest-level, highest-floor and top-earner targets
fall back to the triggering floor when it's the only unlocked one, so Negroni
Nightfall doubles up there rather than paying nothing. Pocket Money is the only
one that touches tiers — a single promotion, so a floor already at the strongest
tier keeps it and still takes its 15 upgrades. None reset a collection timer.
No map-specific rewards are implemented, so all twenty-seven stay out of
`MAP_CRIT_TEST_KINDS`.

Processing: regenerate with `node scripts/process-<name>.mjs` per asset. Every
raw `.jfif` is preserved; the wrappers call `scripts/lib/process-crit-icon.mjs`,
which border-seeds the near-white background (all twenty-seven sample 246-254 at
the edges), tight-crops, caps at 250px, and writes palette-quantized PNGs to
`public/`, `public/stickers/` and `public/silhouettes/`. Cut-outs were inspected
on a contrasting background: ice cubes, citrus wheels, umbrella garnishes, tiki
flames, loose coins and gems all survived the small-component pass.

Verification: `node scripts/test-featured-crits.mjs` passes with 590 rewards
(including Pocket Money's tier assertion), and `npm run build` succeeds.

## Implemented asset batch: 2026-09-22 (cards, dice and last orders)

Nine supplied illustrations support upgrade clicks and floor unlocks through the
shared `applyFloorCrit` path. Rewards are immediate and current-building only;
no existing crit's balance changed. Proc chances apply only after a tier and the
special gateway land, before the shared cap, so they are not per-click odds.

| Image         | Crit             | Immediate reward                               | Proc chance | Comparison                                                        |
| ------------- | ---------------- | ---------------------------------------------- | ----------- | ----------------------------------------------------------------- |
| beerBelly     | Brewmaster's Cut | 23 instant payouts on this floor               | 0.6%        | Under Glaze Runner's 30 at 0.3%, and commoner to match            |
| bottomsUp     | Bottoms Up       | 25 free upgrades on this floor                 | 0.45%       | Just under Buttercream Boom's 26 at 0.4%                          |
| wineCountry   | Merlot Margins   | 27 instant payouts on every other floor        | 0.38%       | Alternating target, between Tap That Asset and Sapphire Hourglass |
| ponyKeg       | Vintage Returns  | 28 payouts on the lowest-level floor           | 0.32%       | Above Core Value's 22 at 0.5% on the same target                  |
| vodkaWhiskers | Top Shelf        | 31 free upgrades on the highest unlocked floor | 0.28%       | Under Arcane Automaton's 34 at 0.2%, same target                  |
| highRoller3   | Hot Dice         | 32 instant payouts on this floor               | 0.26%       | Between Glaze Runner's 30 and Otterly Adorable's 35               |
| splitThePot   | High Roller      | 35 upgrades here and on the highest floor      | 0.22%       | Far above Finger Guns' 2 on the same two-floor split              |
| highRoller2   | Chip Leader      | 37 free upgrades on this floor                 | 0.16%       | Between Gearheart Guardian's 36 at 0.18% and Croc Rampage's 39    |
| pokerNight    | Poker Night      | 39 instant payouts on every unlocked floor     | 0.11%       | Between Shiba Surplus's 38 at 0.12% and Crocodile Cash's 40       |

Naming: the supplied `highRoller.jfif` collided with the already-shipped
`highRoller` crit, whose raw source is `src/assets/highRoller.png` (the
poker-chip stacks). Processing it under that basename would have overwritten a
live icon, so the raw was renamed to `splitThePot.jfif`. The **name** "High
Roller" then moved to this new dice-table cat, and the chip-stack crit became
"Stacked Chips".

Only the player-facing labels were swapped — both kinds, icons and raw sources
stayed put. Badge collection counts persist per KIND
(`shared/critTypes/critProcCounts.ts`), so renaming the `highRoller` kind would
have silently handed that crit's lifetime collection to the new cat. Because of
that, `splitThePot`/`highRoller` are the one pair in the registry whose kind and
label deliberately don't match; the comment on `highRoller` records why.
`highRoller2`/`highRoller3` keep their supplied basenames — same convention as
`mangoMojito2`/`blueLagoonCocktail2` — with their own distinct labels.

Scope: every reward is current-building only. Building-wide payouts (Poker
Night) and alternating payouts (Merlot Margins) skip locked floors. Vintage
Returns, Top Shelf and High Roller fall back to the triggering floor when it
is the only unlocked one, so High Roller doubles up there rather than paying
nothing. None of them touch crit tiers or reset a collection timer, and repeated
targets stack additively with any other proc landing in the same cap-limited
set. No map-specific rewards are implemented, so all nine stay out of
`MAP_CRIT_TEST_KINDS`.

Processing: regenerate with `node scripts/process-<name>.mjs` for each of
`beerBelly`, `bottomsUp`, `highRoller2`, `highRoller3`, `pokerNight`, `ponyKeg`,
`splitThePot`, `vodkaWhiskers`, `wineCountry`. Every raw `.jfif` is preserved;
the wrappers call `scripts/lib/process-crit-icon.mjs`, which border-seeds the
near-white background (all nine sample 247-253 at the edges), tight-crops, caps
at 250px, and writes palette-quantized PNGs to `public/`, `public/stickers/` and
`public/silhouettes/`. Cut-outs were inspected on a contrasting background: the
dice detached from each cat's paws survived the small-component pass, as did the
chip stacks, beer foam and wine-glass stems.

Verification: `node scripts/test-featured-crits.mjs` passes with 563 rewards
(including five new reward/odds-ordering families covering both dice cats and
all three drink targets), and `npm run build` succeeds.

## Implemented asset batch: 2026-09-22 (treats, arcane machines and armour)

Eleven supplied illustrations support upgrade clicks and floor unlocks through
the shared `applyFloorCrit` path. Rewards are immediate and current-building
only; no existing crit's balance changed. Proc chances apply only after a tier
and the special gateway land, before the shared cap, so they are not per-click
odds.

| Image            | Crit               | Immediate reward                                 | Proc chance | Comparison                                                        |
| ---------------- | ------------------ | ------------------------------------------------ | ----------- | ----------------------------------------------------------------- |
| apple            | Core Value         | 22 payouts on the lowest-level floor             | 0.5%        | Pays the same target Check Up upgrades, in cash instead of levels |
| cupcake          | Buttercream Boom   | 26 free upgrades on this floor                   | 0.4%        | Just under Puzzle Box's 27 at 0.25%, and commoner to match        |
| potion           | Vial of Ventures   | 24 free upgrades on every other floor            | 0.35%       | Above The Fast and the Furriest on the same alternating target    |
| donut            | Glaze Runner       | 30 instant payouts on this floor                 | 0.3%        | Above Iceberg Payday's 28 at 0.3%, same target                    |
| clockworkWizard2 | Arcane Automaton   | 34 free upgrades on the highest unlocked floor   | 0.2%        | Above Space Race's 20 on the same top-floor target                |
| metalHeart       | Gearheart Guardian | 36 free upgrades on this floor                   | 0.18%       | Between Cryo Lock's 36 at 0.15% and Quinn's Whirlwind's 32        |
| metalHeart2      | Piston Paladin     | 41 payouts from the highest-earning floor        | 0.14%       | Four times Sharpshooter's 10 on the same top-earner target        |
| fullPlate        | Ironclad Guarantee | 33 upgrades on this floor and every floor below  | 0.25%       | Above Frost Rune's 29 at 0.4% on the same downward slice          |
| clockworkWizard  | Spellsprocket      | 1 tier promotion, then 12 upgrades on this floor | 0.12%       | Blessed promotes once for 3 upgrades at a far commoner rate       |
| bulwark          | Hold the Line      | 43 free upgrades on this floor                   | 0.09%       | Above Breaking Point's 42 at 0.1%, and rarer to match             |
| overlord         | Dread Sovereign    | 45 instant payouts on every unlocked floor       | 0.08%       | Above Crocodile Cash's 40 at 0.1%, the previous building-wide top |

Scope: every reward is current-building only, never company-wide. Building-wide
payouts (Dread Sovereign) and alternating upgrades (Vial of Ventures) skip
locked floors; Ironclad Guarantee walks from the triggering floor down to the
ground floor and lands on just that floor when it fires on the ground floor
itself. Core Value, Arcane Automaton and Piston Paladin fall back to the
triggering floor when it is the only unlocked one. Spellsprocket is the only
one that touches tiers — a single promotion, so a floor already at the strongest
tier just keeps it and still takes the 12 upgrades. None of them reset a
collection timer, and repeated targets stack additively with any other proc
landing in the same cap-limited set. No map-specific rewards are implemented,
so all eleven stay out of `MAP_CRIT_TEST_KINDS`.

Processing: regenerate with `node scripts/process-<name>.mjs` for each of
`apple`, `bulwark`, `clockworkWizard`, `clockworkWizard2`, `cupcake`, `donut`,
`fullPlate`, `metalHeart`, `metalHeart2`, `overlord`, `potion`. Every raw
`.jfif` is preserved; the wrappers call `scripts/lib/process-crit-icon.mjs`,
which border-seeds the near-white background (all eleven sample 248-253 at the
edges), tight-crops, caps at 250px, and writes palette-quantized PNGs to
`public/`, `public/stickers/` and `public/silhouettes/`. Cut-outs were inspected
on a contrasting background: outlines, sparkles, the apple's bite highlight, the
potion's pale glass, and every horn, dagger and boot survived.

`fullPlate` had been committed in an earlier WIP commit but never processed or
registered; it is folded into this batch rather than left orphaned.

Verification: `node scripts/test-featured-crits.mjs` passes with 554 rewards
(including Spellsprocket's tier assertion and five reward/odds-ordering
families), and `npm run build` succeeds.
