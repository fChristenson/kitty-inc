...
// raw src/assets/sportscar.jfif (a flat vector-style blue racing car with
// flame decals, on a plain near-white background, ~242-253 whiteness) gets
// border-flood-fill chroma-keyed to transparent (same technique as
// process-ball.mjs/process-shield.mjs/process-clock.mjs) — the car's own
// white highlights (headlight, hood stripe) sit at a similarly-high
// whiteness as the real background, but each is fully enclosed by the car's
// own continuous black outline, so a border-only flood fill can never leak
// in and clear them by mistake. Backs the "Rush Hour" crit's flash icon.
const WHITE_LO = 210;
const WHITE_HI = 235;
const FLOOD_LO = 210;

const assets = path.resolve(import.meta.dirname, "..", "src", "assets");
const src = path.join(assets, "sportscar.jfif");
const dest = path.join(assets, "sportscar.png");
...
// the car+flames+spoiler are all one continuous outlined shape — discard any
// disconnected fleck the fade above left behind
