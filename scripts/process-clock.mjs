...
// raw src/assets/clock.jfif (AI-generated: a red cartoon wall clock icon on a
// plain cream background, with a soft drop shadow) gets border-flood-fill
// chroma-keyed to transparent (same technique as process-shield.mjs/
// process-coin.mjs), then cropped to its own tight bounding box. Writes to
// src/assets/clock.png; never overwrites the raw source.
//
// Thresholds are shifted down from process-shield.mjs's (which assumed a
// near-pure-white ~255 background) since this source's background/shadow is a
// darker cream (~190-251 whiteness) — verified by sampling raw pixels first.
...
const src = path.join(assets, "clock.jfif");
const dest = path.join(assets, "clock.png");
...
// it as content — same fix process-shield.mjs/process-coin.mjs needed
