...
// Process the Rain Check cat-and-umbrella icon from its near-white background.
// Border flood fill preserves enclosed white highlights inside the cat and
// umbrella while removing the real background, then the icon is shadowed,
// size-capped, and palette-quantized for the flash backdrop.
const WHITE_LO = 190;
const WHITE_HI = 235;
const FLOOD_LO = 190;
...
const src = path.join(assets, "rainCheck.jfif");
const dest = path.join(assets, "rainCheck.png");
...
// The raindrops are real detached components, so keep all meaningful blobs
// instead of retaining only the connected cat-and-umbrella silhouette.
