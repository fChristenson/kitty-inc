...
for (let offset = 0; offset < elementCritBatch.length; offset += 20) {
  ...
  for (const [
    ...
    layers.push({
      input: image,
      left: left + Math.floor((220 - metadata.width) / 2),
      top: top + Math.floor((190 - metadata.height) / 2),
    });
  ...
  const output = path.join(
    destination,
    `${processed ? "processed" : "source"}-${offset / 20 + 1}.png`,
  );
  await sharp({
    create: {
      width: 1100,
      height: Math.ceil(entries.length / 5) * 225,
      channels: 4,
      background: processed ? "#e85de7" : "#dde4e7",
    },
  })
    .composite(layers)
    .png()
