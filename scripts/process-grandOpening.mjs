...
// raw src/assets/grandOpening.jfif (a ceremonial grand-opening ribbon/city
// icon on a plain near-white background) gets border-flood-fill chroma-keyed
// to transparent (same technique as process-upgrade.mjs/process-goldenTicket.
// mjs), then tight-cropped and drop-shadowed. The source has the same JFIF
// edge noise as other icons here, so keepLargestOpaqueComponent removes any
// disconnected flecks left behind by the fade. Final output is palette-
// quantized and capped to 250x250 since this art is only used as a small crit
// flash backdrop. Writes src/assets/grandOpening.png.
const WHITE_LO = 220;
const WHITE_HI = 245;
const FLOOD_LO = 220;

const assets = path.resolve(import.meta.dirname, "..", "src", "assets");
const src = path.join(assets, "grandOpening.jfif");
const dest = path.join(assets, "grandOpening.png");
