...
// raw src/assets/cashBillSprite.jfif (AI-generated: one flutter sprite sheet, plain
// near-white background, 6 poses of a green cash bill tumbling through the air,
// spaced evenly across a single row) gets chroma-keyed to transparent and
// re-composited into a uniform-cell grid — same technique as
// process-coin-sprites.mjs, with each frame centered both horizontally and
// vertically in its cell (a tumbling bill has no shared ground line either). Writes
// to src/assets/themes/references/dist/sprites/cashBillFlutter.png; never overwrites the raw source.
...
const srcFile = path.join(assets, "cashBillSprite.jfif");
const destFile = path.join(outDir, "cashBillFlutter.png");
...
// flood-fill (BFS, 4-connected) starting only from the image border — a real dark
// outline pixel is always way below FLOOD_LO, so white fully enclosed inside a pose
// (e.g. the bill's own "$" mark) is never reached and never stripped
...
// canvas, centered both horizontally and vertically), then lay all cells out left
// to right into the final sheet
