...
// the card faces are themselves a cream off-white (whiteness ~230-245), but
// they're sealed behind the cards' heavy black outline, so a border-seeded
// fill can never reach them — which lets the thresholds sit low enough to eat
// the ragged pale sketch rim drawn around the pair
const WHITE_LO = 195;
const WHITE_HI = 235;
const FLOOD_LO = 195;
const assets = path.resolve(import.meta.dirname, "../../..", "src", "assets", "crits", "gamesOfChance");
const src = path.join(assets, "doubleDown.jfif");
const dest = path.join(assets, "doubleDown.png");
...
// the two cards overlap, so the pair is a single opaque blob
