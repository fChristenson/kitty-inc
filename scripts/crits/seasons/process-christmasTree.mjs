...
// raw src/assets/christmasTree.jfif (AI-generated: a decorated Christmas tree
// with wrapped gifts/bags at its base, on a plain light-gray background) gets
// border-flood-fill chroma-keyed to transparent (see process-pair.mjs's own
// comment for why this technique + dropSmallOpaqueComponents, not
// keepLargestOpaqueComponent, is the right pick \u2014 the tree and its several
// separate gift boxes are a legitimately multi-part scene). Cropped to its
// own tight bounding box, given a synthesized drop shadow, and
// palette-quantized. Writes to src/assets/christmasTree.png; never
// overwrites the raw source. Backs the "Winter Sale" crit's flash icon
// (registered under the "winter" ImageName key).
const WHITE_LO = 150;
const WHITE_HI = 195;
const FLOOD_LO = 150;
// background here is a near-perfectly neutral gray (channel spread <=3); real
// content that's merely bright (cream/white gift-box parts) is still clearly
// tinted (spread 30+) — requiring near-neutrality keeps the flood fill from
// ever crossing into warm-toned content even where it's bright enough to pass
// the whiteness check alone
const SAT_MAX = 20;
const MIN_KEEP_AREA = 200;

const assets = path.resolve(import.meta.dirname, "../../..", "src", "assets", "crits", "seasons");
const src = path.join(assets, "christmasTree.jfif");
const dest = path.join(assets, "christmasTree.png");
...
// the background peeking through the narrow gaps between the tree trunk and
// the gift boxes is fully enclosed by their outlines (no pixel path back to
// the image border survives), so the border-seeded flood fill alone can
// never reach it — these extra seeds (hand-picked by sampling the raw source)
// let it flood outward from inside that pocket too
for (const [sx, sy] of [
  [521, 614],
  [540, 600],
  [560, 595],
...
// between objects (never touching a non-background neighbor), ambient-
