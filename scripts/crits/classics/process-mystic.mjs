...
// Mystic is a near-white-background cat icon with enclosed near-white details
// in the eyes and paws, so the background must be border-flood-filled rather
// than removed by a global whiteness threshold.
const WHITE_LO = 220;
const WHITE_HI = 245;
const FLOOD_LO = 220;

const assets = path.resolve(import.meta.dirname, "../../..", "src", "assets", "crits", "classics");
const src = path.join(assets, "mystic.jfif");
const dest = path.join(assets, "mystic.png");
const shippedDest = path.join(assets, "..", "..", "themes", "references", "dist", "crits", "classics",
  "mystic.png",
);
...
console.log(
  `wrote ${path.relative(assets, dest)} and ${path.relative(assets, shippedDest)}: ${finalMeta.width}x${finalMeta.height}`,
);
