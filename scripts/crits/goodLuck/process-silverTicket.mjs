...
// raw src/assets/silverTicket.jfif (a flat vector-style silver/gray admission
// ticket with a dark navy outline + dashed perforation line, on a plain
// near-white background, ~241-254 whiteness) gets border-flood-fill
// chroma-keyed to transparent (same technique as process-goldenTicket.mjs/
// process-sportscar.mjs) — the ticket's own glossy silver highlight sits
// only a few whiteness units below the real background (a narrower gap than
// most other icons here), but it's fully enclosed by the ticket's own
// continuous dark outline (measured whiteness ~38-75, far below any
// reasonable flood threshold), so a border-only flood fill can never leak in
// and clear it by mistake regardless of how close that highlight's own tone
// sits to the background. Backs the "Silver Ticket" crit's flash icon.
const WHITE_LO = 210;
const WHITE_HI = 235;
const FLOOD_LO = 210;

const assets = path.resolve(import.meta.dirname, "../../..", "src", "assets", "crits", "goodLuck");
const src = path.join(assets, "silverTicket.jfif");
const dest = path.join(assets, "silverTicket.png");
...
// the ticket is one continuous outlined shape — discard any disconnected
// fleck the fade above left behind
...
// opaque pixels per row/column (not a stray low-alpha noise speck)
...
// source art had no baked-in shadow (fully flat vector style) — synthesize
// one to match the other special-crit icons' visual style
