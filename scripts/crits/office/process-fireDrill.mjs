...
// the badge plate's pale panel falls inside these thresholds, so the fill is
// seeded from the border rather than applied per-pixel — it can't cross the
// helmet's heavy outline to reach it. Kept low enough to also eat the ragged
// pale sketch rim drawn around the whole shape
const WHITE_LO = 195;
const WHITE_HI = 235;
const FLOOD_LO = 195;
const assets = path.resolve(import.meta.dirname, "../../..", "src", "assets", "crits", "office");
const src = path.join(assets, "fireDrill.jfif");
const dest = path.join(assets, "fireDrill.png");
