...
// the cat's cream belly/paws and the mug's pale rim fall inside these
// thresholds, so the fill is border-seeded rather than applied per-pixel — it
// can't cross the heavy outline to reach them. Kept low enough to also eat the
// ragged pale sketch rim drawn around the whole scene
const WHITE_LO = 195;
const WHITE_HI = 235;
const FLOOD_LO = 195;
// the mug's handle encloses a pocket of background the border-seeded fill can
// never reach on its own
const EXTRA_FLOOD_SEEDS = [[235, 497]];
// the steam squiggles above the mug are real content floating free of the cat,
// so only genuine chroma-key specks may be dropped
const MIN_KEEP_AREA = 300;
const assets = path.resolve(import.meta.dirname, "../../..", "src", "assets", "crits", "office");
const src = path.join(assets, "coffeeRun.jfif");
const dest = path.join(assets, "coffeeRun.png");
