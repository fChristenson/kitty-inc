...
// raw src/assets/cashRegister.jfif (AI-generated: a vintage brown/gold cartoon
// cash register, on a plain cream background) gets border-flood-fill
// chroma-keyed to transparent (same technique as process-coin.mjs/
// process-graph.mjs), then cropped to its own tight bounding box. Writes to
// src/assets/cashRegister.png; never overwrites the raw source.
const WHITE_LO = 200;
const WHITE_HI = 230;
const FLOOD_LO = 150;
// same fix process-merge.mjs/process-skyscraper.mjs needed: background bleed
// can survive as a low but nonzero alpha, high enough to survive the crop's
// own MIN_OPAQUE_RUN check and show up as visible border residue
const NOISE_ALPHA_FLOOR = 100;

const assets = path.resolve(import.meta.dirname, "..", "src", "assets");
const src = path.join(assets, "cashRegister.jfif");
const dest = path.join(assets, "cashRegister.png");
...
// it as content — same fix process-coin.mjs/process-graph.mjs needed
