...
// raw src/assets/goldenParachute.jfif (AI-generated: a golden-yellow
// parachute canopy + red/blue harness on a plain near-white background) gets
// border-flood-fill chroma-keyed to transparent (same technique as
// process-upgrade.mjs/process-clock.mjs) — the gaps between the canopy's
// suspension lines all touch the image border, so a border-only flood fill
// clears them along with the real background, same as everywhere else with
// no enclosed near-white detail to protect — then cropped to its own tight
// bounding box. The original art's own soft drop shadow gets clipped away
// along with the background by that same chroma-key, so a fresh one is
// synthesized from the cropped icon's alpha silhouette instead. Then
// palette-quantized to keep the shipped file small. Writes to
// src/assets/goldenParachute.png; never overwrites the raw source.
const WHITE_LO = 190;
const WHITE_HI = 235;
const FLOOD_LO = 190;

const assets = path.resolve(import.meta.dirname, "..", "src", "assets");
const src = path.join(assets, "goldenParachute.jfif");
const dest = path.join(assets, "goldenParachute.png");
...
// canopy/harness/lines are always one single connected shape
