...
// raw src/assets/flush.jfif (AI-generated: five fanned playing cards,
// 10-J-Q-K-A of hearts, on a plain near-white background) gets
// border-flood-fill chroma-keyed to transparent (same technique + thresholds
// as process-fullHouse.mjs's own fanned-card scene — a plain WHITE_HI=245-style
// threshold leaves a faint gray backing box visible on this near-white-but-not-
// pure-white background), cropped to its own tight bounding box, given a
// synthesized drop shadow, and palette-quantized. Each card is its own
// component (fanned, not one connected blob), so dropSmallOpaqueComponents
// (not keepLargestOpaqueComponent) keeps every card. Writes to
// src/assets/royalFlush.png; never overwrites the raw source.
const WHITE_LO = 170;
const WHITE_HI = 220;
const FLOOD_LO = 170;
...
const src = path.join(assets, "flush.jfif");
const dest = path.join(assets, "royalFlush.png");
