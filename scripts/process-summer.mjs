...
// raw src/assets/summer.jfif (AI-generated: a beach chair with sunglasses
// under a striped umbrella, on a sandy patch over a plain light-gray
// background) gets border-flood-fill chroma-keyed to transparent (see
// process-pair.mjs's own comment for why this technique +
// dropSmallOpaqueComponents, not keepLargestOpaqueComponent, is the right
// pick — the umbrella and chair are only loosely connected through the sand
// patch beneath them). Cropped to its own tight bounding box, given a
// synthesized drop shadow, and palette-quantized. Writes to
// src/assets/summer.png; never overwrites the raw source. Backs the
// "Summer Sale" crit's flash icon.
const WHITE_LO = 150;
const WHITE_HI = 195;
const FLOOD_LO = 150;
...
const src = path.join(assets, "summer.jfif");
const dest = path.join(assets, "summer.png");
