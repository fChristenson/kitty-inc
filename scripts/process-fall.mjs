...
// raw src/assets/fall.jfif (AI-generated: a small pile/cluster of autumn
// leaves on a plain near-white background) gets border-flood-fill
// chroma-keyed to transparent (see process-pair.mjs's own comment for why
// this technique + dropSmallOpaqueComponents, not keepLargestOpaqueComponent,
// is the right pick — several of the individual leaves are only loosely
// overlapping, not one single connected shape). Cropped to its own tight
// bounding box, given a synthesized drop shadow, and palette-quantized.
// Writes to src/assets/fall.png; never overwrites the raw source. Backs the
// "Autumn Sale" crit's flash icon (registered under the "autumn" ImageName
// key).
const WHITE_LO = 150;
const WHITE_HI = 195;
const FLOOD_LO = 150;
// background here is a near-perfectly neutral near-white (channel spread
// <=3); real content that's merely bright is still clearly tinted (warm leaf
// tones) — requiring near-neutrality keeps the flood fill from ever crossing
// into that real content just because it's bright enough to pass the
// whiteness check alone
const SAT_MAX = 20;
const MIN_KEEP_AREA = 200;

const assets = path.resolve(import.meta.dirname, "..", "src", "assets");
const src = path.join(assets, "fall.jfif");
const dest = path.join(assets, "fall.png");
...
// the background peeking through the gaps in the leaf canopy near the trunk
// split is fully enclosed by the branches/leaves (no pixel path back to the
// image border survives), so the border-seeded flood fill alone can never
// reach it — these extra seeds (hand-picked by sampling the raw source) let
// it flood outward from inside that pocket too
for (const [sx, sy] of [
  [695, 370],
  [680, 390],
  [670, 410],
...
// between leaves (never touching a non-background neighbor), ambient-
