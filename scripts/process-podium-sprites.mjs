...
// raw src/assets/podiumSprites.jfif (AI-generated: one cat-at-a-podium sprite
// sheet, plain near-white background, 5 "speaking" poses spaced across one row,
...
// connectivity-based chroma key process-cat-sprites.mjs uses — a real dark
// outline pixel is always way below FLOOD_LO, so white fully enclosed inside a
// pose (e.g. the cat's own eyes/shirt/podium highlight) is never reached and
// never stripped
...
for (let y = 0; y < height; y++) {
  for (let x = 0; x < width; x++) {
    ...
    // color-decontaminate using a blend factor derived straight from whiteness
    // (not the alpha above): whiteness <= WHITE_LO pixels are forced fully
    // opaque by the formula above, but jfif/jpeg anti-aliasing means many of
    // them are still a light gray blend toward the true (near-black) outline,
    // not real foreground color — left alone, that's exactly the pale halo
    // ring seen around the cat once composited over anything but white
...
// canvas, centered horizontally and bottom-aligned so every pose's podium shares
// the same ground line), then lay all cells out left to right into the final
// sheet
